...
$(document).ready(function(){
	$("#submitBtn2").click(function(){
		if ($("#bankInfo").val()=="" || $("#accountInfo").val()=="" || $("#nameInfo").val()=="") {
			...
			exchangevolumeResult="";
		} else if($("#exchangeVolume").val()>${sessionScope.uvo.mileage}) {
	...
	<div class="row">
		<div class="col-lg-3">
			...
			<c:if test="${sessionScope.uvo.userGroupVO.ugroupNo=='1' }">
				...
				<a
					...
					class="list-group-item">마일리지 내역</a>
			</div>
		</div>
		<!-- /.col-lg-3 -->
		...
		<div class="col-lg-9">
			...
			<div class="card card-block login-form p-5">
				<div class="jumbotron">"Gibu&Take"에서 적립한 마일리지는 입력하신 계좌번호로 5일
					내에 입금됩니다.</div>
				<div class="card-body">
					<!-- 마일리지 충전 -->
					<form id="exchangeMileageForm" method="post"
						action="exchangeMileage.do">
						<input type="hidden" name="mugNo" value="3"> <input
							type="hidden" name="uvo" value="${sessionScope.uvo}">
						<div class="form-group">
							보유 마일리지: ${sessionScope.uvo.mileage} P <br> <input
								type="number" class="form-control" name="mtVolume"
								id="exchangeVolume" placeholder="환전할 금액">
						</div>
						<div class="form-group">
							<label>은행</label> <input type="text" class="form-control"
								id="bankInfo" placeholder="은행" required="required">
						</div>
						<div class="form-group">
							<label>계좌번호</label> <input type="text" class="form-control"
								id="accountInfo" placeholder="계좌번호" required="required">
						</div>
						<div class="form-group">
							<label>예금주 명</label> <input type="text" class="form-control"
								id="nameInfo" placeholder="예금주" required="required">
						</div>
						...
						<br>
						<br>
						<button type="submit" class="btn btn-point btn-block"
							id="submitBtn2">환전</button>
					</form>
				</div>
			</div>
		</div>
